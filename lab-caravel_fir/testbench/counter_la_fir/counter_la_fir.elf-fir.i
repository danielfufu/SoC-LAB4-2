# 0 "fir.c"
# 1 "/home/ubuntu/lab-caravel_fir/testbench/counter_la_fir//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "fir.c"
# 1 "fir.h" 1
...
int* __attribute__ ( ( section ( ".mprjram" ) ) ) fir(){
 ...
 int x[64];
 for(int i = 0 ; i < (*(volatile uint32_t*)0x30000010) ; i++)
  x[i] = i;

 int read_ap = (*(volatile uint32_t*)0x30000000);
 for(int i = 0 ; i < (*(volatile uint32_t*)0x30000010) ; i++){
  while(i>0 && !((read_ap)>>4 & 1) )
   read_ap = (*(volatile uint32_t*)0x30000000);
  ...
  ans[i] = (*(volatile uint32_t*)0x30000084);

 }
 read_ap = (*(volatile uint32_t*)0x30000000);

 (*(volatile uint32_t*)0x2600000c) = (ans[63] << 24) | 0x005A0000 ;
